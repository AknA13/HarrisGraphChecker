			...
			if (vertexPlacingButton.isSelected()) {
						...
						if (vertices.get(i).getLayoutBounds().contains(event.getX(), event.getY())) {
							...
							for(int j = 0 ; j < edgeMatrix.size(); j ++) {
								for(int k = 0 ; k < edgeMatrix.get(j).size(); k ++) {
									if(edgeMatrix.get(j).get(k)!=null && root.getChildren().contains(edgeMatrix.get(j).get(k))) {
							...
							for(int j = 0 ; j < graph.size(); j ++) {
								for(int k = 0 ; k < graph.get(j).size(); k++) {
									edgeMatrix.get(j).set(graph.get(j).get(k), new Line(vertices.get(j).getCenterX(),vertices.get(j).getCenterY(),vertices.get(graph.get(j).get(k)).getCenterX(),vertices.get(graph.get(j).get(k)).getCenterY()));
									edgeMatrix.get(graph.get(j).get(k)).set(j, new Line(vertices.get(j).getCenterX(),vertices.get(j).getCenterY(),vertices.get(graph.get(j).get(k)).getCenterX(),vertices.get(graph.get(j).get(k)).getCenterY()));
								}
							}
							for(int j = 0 ; j < edgeMatrix.size(); j ++) {
				...
				if (event.getButton().equals(MouseButton.PRIMARY)) {
					...
					if (event.getEventType().equals(MouseEvent.MOUSE_RELEASED)) {
						for (int i = 0; i < vertices.size(); i++) {
							if (vertices.get(i).getLayoutBounds().contains(event.getX(), event.getY())) {
								if (currNodeFrom != vertices.get(i)) {
					...
					for (int i = 0; i < edgeMatrix.size(); i++) {
						for (int j = 0; j < edgeMatrix.get(i).size(); j++) {
							if (edgeMatrix.get(i).get(j) != null) {
								if (edgeMatrix.get(i).get(j).getLayoutBounds().contains(event.getX(), event.getY())) {
									root.getChildren().remove(edgeMatrix.get(i).get(j));
									edgeMatrix.get(j).set(i, null);
									edgeMatrix.get(i).set(j, null);
									graph.get(i).remove(new Integer(j));
